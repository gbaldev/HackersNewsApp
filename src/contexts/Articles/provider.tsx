...
export const fetchArticles: (interests?: {
  ios: boolean;
  android: boolean;
}) => Promise<ArticleResponse> = async interests => {
  let response;
  if (interests) {
    let queryParams = '';
...
export const ArticlesProvider: React.ComponentType<ArticlesProviderProps> = ({
  ...
  const favorites = useMemo(
    () => articles.filter(article => article.isFavorite && !article.isDeleted),
    [articles],
  );
  const deleted = useMemo(
    () => articles.filter(article => article.isDeleted),
    [articles],
  );
  ...
  useEffect(() => {
    const initialLoading = async () => {
      ...
      await AsyncStorage.setItem(
        'articles',
        JSON.stringify(normalizedArticles),
      );
      await AsyncStorage.setItem(
        'lastCreatedArticle',
        normalizedArticles[0].created_at,
      );
      setIsLoading(false);
    };
    initialLoading();
  ...
  const contextValue = useMemo<ArticleContextType>(
    ...
    [articles, onFavorite, favorites, deleted, onRefresh, onDelete, isLoading],
  );
