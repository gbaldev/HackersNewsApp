...
const ArticleCard: ComponentType<ArticleCardProps> = ({ article, onFavorite, onDelete, disableScroll, enableScroll }) => {
  ...
  const createdAt = useMemo(
    () => {
      ...
      const diffMinutes = Math.abs(currentDate.diff(createdAt, 'minutes').minutes);
    ...
    <>
      ...
      >
        <View style={styles.card}>
          <TouchableOpacity
            onPress={() => navigation.navigate('WEBVIEW', { uri: article.story_url, title: article.story_title })}
            style={[styles.innerCard, favoriteStyle]}>
            <View style={styles.header}>
              <View style={styles.titleContainer}>
                <Text style={styles.titleText}>{article.story_title}</Text>
                {!article.isDeleted && <TouchableOpacity onPress={() => onFavorite(article.objectID)}>
                  <Icon name={!article.isFavorite ? 'favorite' : 'favoriteActive'} />
                </TouchableOpacity>}
              </View>
            </View>
            <View style={styles.titleContainer}>
