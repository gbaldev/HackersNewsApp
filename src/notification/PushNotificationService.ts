...
export const initBackgroundFetching = () => {
  BackgroundFetch.configure(
    ...
    async taskId => {
      ...
      let interests: string | {ios: boolean; android: boolean} | null =
        await AsyncStorage.getItem('pushPreferences');
      interests = (
        typeof interests === 'string'
          ? JSON.parse(interests)
          : {ios: true, android: true}
      ) as {ios: boolean; android: boolean};
      ...
      const lastArticleCreatedAt = await AsyncStorage.getItem(
        'lastCreatedArticle',
      );
      let normalizedArticles = fetchedArticles.hits
        .filter(h => !!h.story_url || !!h.url)
        .map(hit => normalizeHit(hit));
      if (lastArticleCreatedAt) {
        const lastArticleCreatedAtDate = DateTime.fromJSDate(
          new Date(lastArticleCreatedAt),
        );

        // keep articles after the last article created (new ones).
        normalizedArticles = normalizedArticles.filter(article => {
          const newArticleDate = DateTime.fromJSDate(
      ...
      for (const article of normalizedArticles) {
        const isAlreadyFetched = articles.some(
          _article => article.objectID === _article.objectID,
        );
        if (isAlreadyFetched) {
          continue;
        } else {
  ...
  Notifications.events().registerNotificationOpened(
    (notification: Notification, completion) => {
      navigation.navigate(StackRoutes.WEBVIEW, {
        uri: notification.payload.data.uri,
        title: notification.payload.data.title,
      });
      completion();
    },
  );
